...
if (!isset($_SESSION['user_id']) || AuthHelper::getRole() !== 'owner') {
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'create_property') {
            ...
            $platformName = trim($_POST['SAAS_PLATFORM_NAME'] ?? '');
            $supportEmail = trim($_POST['SAAS_SUPPORT_EMAIL'] ?? '');
            $trialDays = (int)($_POST['SAAS_TRIAL_DAYS'] ?? 30);
            $defaultCurrency = trim($_POST['SAAS_DEFAULT_CURRENCY'] ?? 'INR');
            $saasSubdomain = trim($_POST['SAAS_PORTAL_SUBDOMAIN'] ?? '');

            try {
                $stmt = $db->prepare("INSERT INTO system_settings (key_name, key_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE key_value = VALUES(key_value)");
                $stmt->execute(['SAAS_PLATFORM_NAME', $platformName]);
                $stmt->execute(['SAAS_SUPPORT_EMAIL', $supportEmail]);
                $stmt->execute(['SAAS_TRIAL_DAYS', (string)$trialDays]);
                $stmt->execute(['SAAS_DEFAULT_CURRENCY', $defaultCurrency]);
                $stmt->execute(['SAAS_PORTAL_SUBDOMAIN', $saasSubdomain]);

                $message = "SaaS Platform Settings updated successfully!";
            } catch (\Exception $e) {
                $error = 'Failed to update SaaS configurations: ' . $e->getMessage();
            }
...
$saasPlatformName = 'MicroPMS SaaS Platform';
$saasSupportEmail = 'support@micropms.com';
$saasTrialDays = 30;
$saasDefaultCurrency = 'INR';
$saasPortalSubdomain = '';

try {
    $pName = $db->query("SELECT key_value FROM system_settings WHERE key_name = 'SAAS_PLATFORM_NAME'")->fetchColumn();
    if ($pName !== false) $saasPlatformName = $pName;

    $sEmail = $db->query("SELECT key_value FROM system_settings WHERE key_name = 'SAAS_SUPPORT_EMAIL'")->fetchColumn();
    if ($sEmail !== false) $saasSupportEmail = $sEmail;

    $tDays = $db->query("SELECT key_value FROM system_settings WHERE key_name = 'SAAS_TRIAL_DAYS'")->fetchColumn();
    if ($tDays !== false) $saasTrialDays = (int)$tDays;

    $dCurr = $db->query("SELECT key_value FROM system_settings WHERE key_name = 'SAAS_DEFAULT_CURRENCY'")->fetchColumn();
    if ($dCurr !== false) $saasDefaultCurrency = $dCurr;

    $sSub = $db->query("SELECT key_value FROM system_settings WHERE key_name = 'SAAS_PORTAL_SUBDOMAIN'")->fetchColumn();
    if ($sSub !== false) $saasPortalSubdomain = $sSub;
} catch (\Exception $ex) {
    // Ignore setting resolution issues if system_settings is locked
}
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-800">
            ...
            <div class="flex items-center gap-3">
                <a href="../../index.php" class="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-xl text-sm transition flex items-center gap-2">
                    <i class="ph ph-arrow-left text-base"></i> Back to PMS
                </a>
            </div>
        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="plan" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none">
                                <option value="starter">Starter (Max 15 Rooms)</option>
                                <option value="pro">Pro (Max 50 Rooms)</option>
                                <option value="enterprise">Enterprise (Unlimited Rooms)</option>
                            </select>
                        </div>
                        <div>
        ...
        <div id="content-settings" class="tab-content hidden">
            <div class="glass-panel p-6 rounded-2xl max-w-2xl mx-auto">
                <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2">
                    <i class="ph ph-gear text-sky-400"></i>
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="SAAS_PLATFORM_NAME" value="<?= htmlspecialchars($saasPlatformName) ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none font-bold">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Support Email</label>
                        <input type="email" name="SAAS_SUPPORT_EMAIL" value="<?= htmlspecialchars($saasSupportEmail) ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none font-bold">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Designated SaaS Domain/Subdomain (Optional)</label>
                        <input type="text" name="SAAS_PORTAL_SUBDOMAIN" value="<?= htmlspecialchars($saasPortalSubdomain) ?>" placeholder="saas.yourdomain.com" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none font-mono">
                        <p class="text-[10px] text-slate-500 mt-1">If set, the Super-Admin portal will return 404 on any other domain.</p>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Trial Period (Days)</label>
                            <input type="number" name="SAAS_TRIAL_DAYS" value="<?= $saasTrialDays ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none font-bold">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Default Currency</label>
                            <input type="text" name="SAAS_DEFAULT_CURRENCY" value="<?= htmlspecialchars($saasDefaultCurrency) ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none font-bold">
                        </div>
                    </div>

                    <button type="submit" class="w-full py-3 bg-slate-900 hover:bg-slate-800 text-sky-400 font-bold rounded-xl text-sm border border-slate-800 transition">
            ...
            <div class="glass-panel p-6 rounded-2xl max-w-2xl mx-auto mt-6">
                <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2">
                    <i class="ph ph-sliders text-emerald-400"></i>
