...
$id = $_GET['id'] ?? null;
if (!$id) render_error_page('Missing Booking ID', 'A booking ID is required to view the invoice.', 400);
...
<body class="min-h-screen py-8 px-4 sm:px-6 lg:px-8 bg-slate-50 print:bg-white print:py-0 print:px-0 text-slate-900 font-sans">
    ...
    <div class="max-w-3xl mx-auto mb-8 flex justify-between items-center print:hidden bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
        <a href="folio.php?id=<?= $booking['id'] ?>" class="flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">
        ...
        <button onclick="window.print()" class="bg-slate-900 hover:bg-slate-800 text-white font-medium px-4 py-2 rounded-lg transition-colors flex items-center gap-2">
            <i class="ph ph-printer text-lg"></i> Print Invoice
        </button>
    </div>
        ...
        <div class="flex flex-col sm:flex-row print:flex-row justify-between items-start pb-12 print:pb-4 border-b border-slate-200">
            <div class="flex flex-col">
                <?php if (defined('PROPERTY_LOGO_BASE64') && !empty(PROPERTY_LOGO_BASE64)): ?>
                    <img src="data:image/png;base64,<?= htmlspecialchars(PROPERTY_LOGO_BASE64) ?>" class="w-24 sm:w-28 print:w-24 h-auto object-contain object-left -ml-1 sm:-ml-2 -mt-1 sm:-mt-2">
                <?php else: ?>
                    <div class="flex items-center gap-3">
                        ...
                        <h1 class="text-2xl font-bold tracking-tight text-slate-900 mb-2"><?= htmlspecialchars(defined('PROPERTY_NAME') && !empty(PROPERTY_NAME) ? PROPERTY_NAME : $hotelName) ?></h1>
                    </div>
                <?php endif; ?>
            ...
            <div class="text-left sm:text-right print:text-right mt-8 sm:mt-0 print:mt-0 relative">
                <h2 class="text-3xl font-light text-slate-300 uppercase tracking-widest mb-4"><?= $taxEnabled ? 'Tax Receipt' : 'Receipt' ?></h2>
                ...
                <tbody class="text-sm">
                    ...
                    <tr class="break-inside-avoid">
                        ...
                        <td class="py-4 print:py-2 border-b border-slate-100 align-top">
                            <span class="font-medium text-slate-900 block"><?= htmlspecialchars($l['description']) ?></span>
                            <?php if(!empty($l['payment_method'])): ?>
                                <span class="inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wider bg-slate-100 text-slate-500"><?= htmlspecialchars($l['payment_method']) ?></span>
                            <?php endif; ?>
                            <?php if($amt < 0): ?>
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <?php if ($balance > 0): ?>
                        ...
                        <div class="flex justify-between items-center pt-2">
                            ...
                            <span class="text-2xl print:text-xl font-bold font-mono text-slate-900">₹<?= number_format(max(0, $balance), 2) ?></span>
                        </div>
                    <?php else: ?>
                        <div class="flex justify-between text-slate-500 border-b border-slate-200 pb-3">
                        ...
                        <div class="flex justify-between items-center pt-2">
                            ...
                            <span class="text-2xl print:text-xl font-bold font-mono text-emerald-600">₹<?= number_format($totalPayments, 2) ?></span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
