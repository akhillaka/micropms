...
const UI = {
    ...
    setLoading: (btnElement, isLoading, originalText = '') => {
        ...
        if (isLoading) {
            ...
            btnElement.innerHTML = `<span class="pms-btn-spinner" aria-hidden="true"></span> Processing...`;
            btnElement.classList.add('opacity-75', 'cursor-not-allowed');
        } else {
            btnElement.disabled = false;
            btnElement.innerHTML = btnElement.dataset.originalHtml || originalText;
            btnElement.classList.remove('opacity-75', 'cursor-not-allowed');
        }
    },

    withButtonLoading: (btn, promise, label) => {
        if (window.ApiClient && typeof ApiClient.withButtonLoading === 'function') {
            return ApiClient.withButtonLoading(btn, promise, label);
        }
        UI.setLoading(btn, true);
        return Promise.resolve(promise).finally(() => UI.setLoading(btn, false));
    },
