...
class RoomService {
     ...
     */
    public static function getAvailable(\PDO $db, int $propertyId, string $checkIn, string $checkOut): array {
        $sql = "SELECT r.*, c.name as category_name
                FROM rooms r
                JOIN room_categories c ON r.category_id = c.id
                WHERE r.property_id = :pid
                  AND r.state != 'out_of_order'
                  AND r.id NOT IN (
                      SELECT b.room_id FROM bookings b
                      WHERE b.property_id = :pid2
     ...
     */
    public static function markClean(\PDO $db, int $roomId, int $propertyId): bool {
     ...
     */
    public static function markDirty(\PDO $db, int $roomId, int $propertyId): bool {
        $stmt = $db->prepare("UPDATE rooms SET state = 'dirty' WHERE id = :id AND property_id = :pid");
        $stmt->execute(['id' => $roomId, 'pid' => $propertyId]);
        return $stmt->rowCount() > 0;
    }
     ...
     */
    public static function getById(\PDO $db, int $roomId, int $propertyId): ?array {
        $stmt = $db->prepare("
            SELECT r.*, c.name as category_name 
            FROM rooms r 
            JOIN room_categories c ON r.category_id = c.id 
