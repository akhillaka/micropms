...
ApiHandler::run(function (\PDO $db) {
    ...
    if ($action === 'list') {
        ...
        ");
        $stmt->execute($params);
        ApiResponse::success(['chats' => $stmt->fetchAll()]);
    }

    if ($action === 'messages') {
        $conv_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
        if (!$conv_id) {
            ApiResponse::error('No conversation ID provided');
        }
        ...
        ApiResponse::success(['messages' => $messages, 'info' => $conv]);
    }
    ...
    if ($action === 'unread_count') {
        ...
        ApiResponse::success(['unread' => (int)$stmt->fetchColumn()]);
    }
